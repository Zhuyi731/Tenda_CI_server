  ...
  <div class="container-box">
    ...
    >
        ...
        <template slot-scope="props">
          ...
          >
            ...
            <el-form-item label="PL/导师" prop="teacher">
              <el-tag type="small" v-for="tag in props.row.teacher" :key="tag">{{members[tag]}}</el-tag>
            </el-form-item>
            ...
            <el-form-item label="审批意见" prop>
              <el-input v-model="props.row.opinion"></el-input>
            </el-form-item>
            <el-form-item label="checklist表" prop>
      ...
      <el-table-column label="操作">
        <template slot-scope="scope" v-if="status == '结束'" >
          <el-button v-if = "scope.row.status == '待审核' && !RegExp(name).test(scope.row.response.join(','))" size="mini" type="primary" @click="handlePass(scope.$index, scope.row)">通过</el-button>
          <el-button v-if = "scope.row.status == '待审核' && !RegExp(name).test(scope.row.response.join(','))" size="mini" type="danger" @click="handleReject(scope.$index, scope.row)">驳回</el-button>
          <el-button v-if = "scope.row.status == '待提交' && !RegExp(name).test(scope.row.submit)" size="mini" type="primary" @click="handleSubmit(scope.$index, scope.row)">提交</el-button>
          <el-button v-else size="mini" disabled >待他人提交</el-button>
        </template>
      </el-table-column>
    </el-table>
  </div>
...
export default {
    ...
    return {
      ...
      checklistData: [
        ...
        },
        {
          ...
          step:
            "进入DUT，将设备恢复出厂设置。以单项功能为颗粒度，检查业务逻辑与原型上的逻辑一致性。单个功能至少测试2种以上的场景，是否存在问题如SSID扫描功能：成功，扫描中、扫描失败3种场景。Wifi开关功能：WIFI开启、关闭场景",
          Result: "与原型一致，符合需求",
          ifchecked: "",
          response: "",
          remarks: ""
        },
        {
          ...
          step:
            "进入DUT，将设备恢复出厂设置。检查各项功能规格与软件规格一致性，且至少测试2种以上的场景。如添加MAC过滤（最多添加16条）：测试添加0条，1条，17条的场景。如SSID设置：测试输入为空，特殊字符，超长字符的场景",
        ...
        },
        {
          ...
          step:
            "打开compare.exe程序所在目录1、将各JSON语言包拖至文件夹下，命名为translate-xx.json(xx为语言代码)2、将xlsx文件拖入。文件第一列为英文，其它列为各国语言执行cmd>compare.exe，查看result.txt文件",
  ...
  methods: {
      ...
      Promise.all([this.$http.post("/api/CI/getHandleList",{status:that.status})]).then(res => {
        ...
        for (var item of that.tableData) {
          ...
          for (i in item.response) {
            item.response[i] = that.members[item.response[i]] + " ";
          }
          item.teacher = item.teacher.split(",");

          for (var i = 0; i < 18; i++) {
            item.checklistData[i].checklistDetail =
              that.checklistData[i].checklistDetail;
    ...
    showOptions:function(submit ){
      
    },
    handlePass: function(index, row) {
      var submitData = { id: row.id, status: "ending", opinion: row.opinion, response:row.response,name:row.name };
    ...
    handleReject: function(index, row) {
      var submitData = { id: row.id, status: "resubmit", opinion: row.opinion };
      this.$http.post("/api/CI/handleProcedure", submitData).then(res => {
        this.notify(res.data);
        this.getHandleList();
      });
    ...
    handleSubmit: function(index,row){
      var submitData = { id: row.id, status: "resubmit", opinion: row.opinion,submit:row.submit};
