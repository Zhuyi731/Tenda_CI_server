...
function CIControl() {
    ...
    this.getAllLine = () => {
        ...
        return new Promise((resolve, reject) => {
                ...
                Promise.all([db.query("SELECT DISTINCT productLine FROM product", ""), db.query("SELECT mail,name FROM users", "")]).then((values) => {
        ...
        this.newProLine = (args) => {
            ...
            return new Promise((resolve) => {
                ...
                //插入新项目之后便要开启项目的检查
                db.query(insertSQL, insertArgs).then((err, rows) => {
                    if (err) {
                        resolve({
                            message: "datebase error"
                        });
